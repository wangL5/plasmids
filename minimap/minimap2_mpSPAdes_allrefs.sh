#!/bin/bash -l
#SBATCH --job-name=minimap_CAMI2_all_refs_mpSPAdes
...
#SBATCH --array=1-10
#SBATCH --output=slurm_minimap_CAMI2_all_refs_mpSPAdes_%A_%a.out
...
ASSEMBLY="${ASSEMBLIES}/CAMI2_${SAMPLE_ID}_mpSPAdes/scaffolds.fasta"

# run minimap2
for FASTA in "${REF_G}"/*.fasta; do
    REF_NAME=$(basename "$FASTA" .fasta)
    OUTPUT="${WORKDIR}/CAMI2_CE_mpSPAdes_${SAMPLE_ID}_${REF_NAME}.paf"
    minimap2 -x asm5 -t "${SLURM_CPUS_PER_TASK}" "$FASTA" "$ASSEMBLY" > "$OUTPUT"
done

echo "All minimap2 jobs completed for mpSPAdes assemblies"